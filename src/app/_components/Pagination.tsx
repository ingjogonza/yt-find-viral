...
export function Pagination({ filters, page, totalPages, totalMatching }: PaginationProps) {
  if (totalPages <= 1) {
    return (
  ...
  const prevHref = page > 1 ? buildCatalogHref(filters, { page: page - 1 }) : null;
  const nextHref = page < totalPages ? buildCatalogHref(filters, { page: page + 1 }) : null;
